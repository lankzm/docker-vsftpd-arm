# dotkevinwong/vsftpd-arm
<em>Forked from [fauria/docker-vsftpd](https://github.com/fauria/docker-vsftpd)</em>

![docker_logo](https://raw.githubusercontent.com/dotkevinwong/docker-vsftpd/master/docker_139x115.png)

This Docker container implements a vsftpd server, with the following features:

 * arm64 CentOS 7  base image
 * vsftpd 3.0
 * Virtual users
 * Passive mode
 * Logging to a file or STDOUT
 * Optimized performance for aarch systems (eg. Raspberry Pi and Apple Silicon Macs)

### Installation from [Docker registry hub](https://registry.hub.docker.com/r/dotkevinwong/vsftpd-arm/).

You can download the image with the following command:

```bash
docker pull dotkevinwong/vsftpd-arm
```
...
```bash
  docker run --rm dotkevinwong/vsftpd-arm
```

2) Create a container in active mode using the default user account, with a binded data directory:

```bash
docker run -d -p 21:21 -v /my/data/directory:/home/vsftpd --name vsftpd dotkevinwong/vsftpd-arm
...
--name vsftpd --restart=always dotkevinwong/vsftpd-arm
```
